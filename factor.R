...
read2=read.table(file="ex.csv",sep=",",header=T)

date=read[,1]
dateall=as.Date(format(date),"%Y%m%d")
uk=read2[,2]
plot(dateall,uk,type="l",ylab="GBP/USD",main="GBP/USD Exchange Rate")

xx=read[1:354,]
index=read2[1:354,6]#6 index,2 exchange rate
date=xx[,1]
datef=as.Date(format(date),"%Y%m%d")
xname=xx[,3:366]
x=as.matrix(cbind(xname))

#plot(datef,index,type="l",ylab="GBP/USD",main="GBP/USD Exchange Rate")
...
legend(datef[250],900,legend=c("Fitted value","Index"),lty=c(1,1),col=c("red","blue"),cex=0.65)
...
for(j in 1:nn)
{
m2[j]=(1:364)[names(xnameback)==name[j]]
port=port+ xnameback[,m2[j]]*out10$coef[j+1]
}
port= port+out10$coef[1]

par(mfrow=c(1,2))
plot(datefback,indexback,type="l",col="red",xlab="Time",ylab="Index",ylim=c(480,900))
title("Tracking Portfolio vs Index (2007-2011)")
lines(datefback,port,type="l",col="blue")
legend(datefback[175],600,legend=c("Index","Tracking Portfolio"),lty=c(1,1),col=c("red","blue"),cex=0.8)
