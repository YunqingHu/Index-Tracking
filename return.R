...
#xx=log(read[,2:ncol(xx)])
xx=read
nr=nrow(xx)
nc=ncol(xx)
ret=mat.or.vec(nr-1,nc)
for (i in 1:nr-1)
{
	for(j in 1:nc)
	{
		ret[i,j]=(xx[i+1,j]-xx[i,j])/xx[i,j]
	}
}
ret[,1]=xx[1:nr-1,1]
dimnames(ret)=dimnames(xx[2:604,])
...
#out11$rsq
#out11$adjr2
plot(out11$adjr2,main='Adj R^2 vs # of stocks',xlab="# of stocks",xlim=c(0,50),ylim=c(0.7,1),ylab="Adjusted R^2",type="l")
#out11$cp
...
plot(datef,out10$res/out10$fit*100,type="p",xlab="Time",ylab="Residuals/Fitted(%)",ylim=c(-100,100))
...
for(j in 1:nn)
{
port=port+ xnameback[,mm[j]]*out10$coef[j+1]
}
port= port+out10$coef[1]

indexback[1]=indexback[1]+1
port[1]=port[1]+1
for(k in 2:251)
{
	port[k]=(1+port[k])*port[k-1]
	indexback[k]=(1+indexback[k])*indexback[k-1]
}
